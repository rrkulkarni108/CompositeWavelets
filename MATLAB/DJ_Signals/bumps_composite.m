...
nl=1; % if using nl = 10 we run into problem with wavmat (W1 and W2 are not orthogonal!)       
...
for m = 1:M 
     ...
     wd_W1_raw = wd_W1; % keep raw copy for BAMS/ABE 
 ...
 fprintf('|------------------------|-------------|------------------|\n');
 fprintf('| %-22s | %.4f      | %.4f           |\n', 'Kron Product (ABE)', avg_mse_kron_ABE, var(mse_kron_ABE_vec));
 ...
 fprintf('| %-22s | %.4f      | %.4f           |\n', 'W2 Daub 3 (ABE)',          avg_mse_W2_ABE,   var(mse_W2_ABE_vec));
 fprintf('-----------------------------------------------------------------------\n');

 fprintf('\nBAMS THRESHOLDING-----------------------------------------------------\n');
 fprintf('| %-22s | %-11s | %-16s |\n', 'Method', 'Average MSE', 'Variance of MSE');
 fprintf('|------------------------|-------------|------------------|\n');
 fprintf('| %-22s | %.4f      | %.4f           |\n', 'Kron Product (BAMS)', avg_mse_kron_BAMS, var(mse_kron_BAMS_vec));
 ...
 fprintf('| %-22s | %.4f      | %.4f           |\n', 'W2 Daub3 (BAMS)',          avg_mse_W2_BAMS,   var(mse_W2_BAMS_vec));
 fprintf('-----------------------------------------------------------------------\n');

 fprintf('\nUNIVERSAL THRESHOLDING------------------------------------------------------\n');
 fprintf('| %-22s | %-11s | %-16s |\n', 'Method', 'Average MSE', 'Variance of MSE');
 fprintf('|------------------------|-------------|------------------|\n');
 fprintf('| %-22s | %.4f      | %.4f           |\n', 'Kron Product', avg_mse_kron, var(mse_kron_vec));
 ...
 fprintf('| %-22s | %.4f      | %.4f           |\n', 'W2 (Daub 3)', avg_mse_W2, var(mse_W2_vec));
 ...
 boxplot([mse_kron_ABE_vec, mse_121_ABE_vec, mse_W1W2_ABE_vec, mse_W1_ABE_vec, mse_W2_ABE_vec], ...
     {'Kronecker', 'W1W2W1', 'W1W2', 'Haar', 'Daubechies 3'});
 ylabel('MSE');
